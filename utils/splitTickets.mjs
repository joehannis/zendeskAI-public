...
const splitTickets = async (ai, obj, initialChunkCount) => {
  console.log(`initialChunkCount: ${initialChunkCount}`);
  ...
  while (!allChunksValid) {
    ...
    for (let i = 0; i < originalTicketCount; i += chunkSize) {
      const chunkedTickets = obj['tickets'].slice(i, i + chunkSize);
    ...
    for (const chunk of tempChunksForValidation) {
      ...
      try {
        const tokenEstimateResponse = await ai.models.countTokens({
          model: 'gemini-2.5-pro', // Or whichever model you are using
