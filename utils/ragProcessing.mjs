...
export const processHierarchically = async (ai, originalData, type) => {
  ...
  const fullEmbedding = await generateEmbeddings(ai, fullPlainText);
  ...
  const sections = await Promise.all(
    sectionsData.map(async (section, index) => {
      const sectionEmbedding = await generateEmbeddings(ai, section.plain_text);
      if (!sectionEmbedding) {
        console.warn(
      ...
      const subChunkObjects = await Promise.all(
        subChunks.map(async (subChunk, subIndex) => {
          ...
          const subChunkEmbedding = await generateEmbeddings(ai, subChunkText);
          if (!subChunkEmbedding) {
            console.warn(
          ...
          return {
            ...
            chunkEmbedding: subChunkEmbedding,
            chunkIndex: subIndex,
          };
        })
      );
  ...
  if (type === 'doc') {
    return {
      ...
      fullDocEmbedding: fullEmbedding,
      sections: sections,
    };
    ...
    return {
      ...
      fullArticleEmbedding: fullEmbedding,
      sections: sections,
    };
  }
};
