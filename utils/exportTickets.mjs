...
const exportTickets = async(ticketsObj) => {
  const outputFilePath = path.join(__dirname, `../../database/csvExports/${ticketsObj[0].tpa ? ticketsObj[0].tpa : ticketsObj[0].tpsa}.csv`);
  ...
  const csvData = ticketsObj[0].tickets.map((item) => {
    const scrapedUrl = `https://pendo.zendesk.com/agent/tickets/${item.id}`
